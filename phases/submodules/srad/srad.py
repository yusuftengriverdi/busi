...
# REF: https://github.com/woshimami/srad/blob/master/srad.py
...
def Gradient(img):
    ...
    #img(i,j+1) - img(i,j)
    grx = img_left - img  + 1e-20
    #img(i+1,j) - img(i,j)
    gry = img_up - img + 1e-20
    #img(i,j)  - img(i,j-1)
    glx = img - img_right + 1e-20
    #img(i,j)   - img(i-1,j)
    gly = img - img_down   + 1e-20
    #img(i,j+1) + img(i+1,j)+ img(i,j-1) +img(i-1,j)  - 4*I(i,j)
    grc = grx+gry-glx-gly  + 1e-20
